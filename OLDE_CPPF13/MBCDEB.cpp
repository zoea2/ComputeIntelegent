	...
	for(int p = 1;p <= POPSIZE;p++){
		...
		for(int i = 0; i < Genotype::NVARS;i++){
			if(temp1.fitness < temp2.fitness){
				...
				MidPop[p].gene[i] = bestA.gene[i] + Genotype::SCALE * (temp2.gene[i] - temp1.gene[i]);
				if(MidPop[p].gene[i] < MidPop[p].lower[i] || MidPop[p].gene[i] > MidPop[p].upper[i])
					MidPop[p].gene[i] = ranval() * (MidPop[p].upper[i] - MidPop[p].lower[i]) + MidPop[p].lower[i];
...
int main(){
	...
	for(f = 1;f <= 13;f++){
		...
		sprintf(filename,"/home/ryan/testdataNew/data_Scale%.1lf/MBCB2013%.1lf/outputc%d.txt",Genotype::SCALE,Genotype::PXOVER,f);
