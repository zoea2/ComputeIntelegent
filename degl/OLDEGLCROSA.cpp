...
const int row = (int)(pow(2.0,ceil(log(Genotype::NVARS+3) / log(2))));
const int col = row - 1;
int **oArray;
int feNumber;
int f;
int bestIdx;
int gen;
const int maxGen = funcEvaluate / POPSIZE;
ofstream output;
Genotype bestA;
...
void init(){
	...
	for(int i = 1;i <= POPSIZE;i++){
		for(int j = 0;j <= Genotype::NVARS+2;j++){
			if(j < Genotype::NVARS){
			...
			else if (j == Genotype::NVARS+1){
				population[i].lower[j] = 0.5;
				population[i].upper[j] = 0.7;
				MidPop[i].lower[j] = 0.5;
				MidPop[i].upper[j] = 0.7;
		
			}
			else if (j == Genotype::NVARS+2){
				population[i].lower[j] = 5;
				population[i].upper[j] = 30;
				MidPop[i].lower[j] = 5;
				MidPop[i].upper[j] = 30;
			}
		...
		population[i].useOrthCount = true;
		if(i == 1){
...
void select(){
	for(int i = 1;i <= POPSIZE;i++){
		if(MidPop[i].fitness < population[i].fitness){
			...
			if((population[i].fitness != 0) && abs((MidPop[i].fitness - population[i].fitness) / population[i].fitness) * 
					gen	/	maxGen	> population[i].updateRate){
				population[i].updateRate = 
					abs((MidPop[i].fitness - population[i].fitness) / population[i].fitness) * gen / maxGen;
				...
				if(population[i].useOrthCount == true){
					population[i].gene[Genotype::NVARS+2] = MidPop[i].gene[Genotype::NVARS+2];
					//output<<"update "<<"gen "<<gen<<"orthcount "<<population[i].gene[Genotype::NVARS+2]<<endl;
			...
			if(population[i].fitness < bestA.fitness){
				...
				bestIdx = i;
				bestA.updateRate = population[i].updateRate;
			}
...
void orthMutate_LGNB(){
	for(int p = 1;p <= POPSIZE;p++){
		...
		if(population[p].useOrthCount == true){
			MidPop[p].gene[Genotype::NVARS+2] = population[p].gene[Genotype::NVARS+2] + population[p].gene[Genotype::NVARS+1] * 
			(population[p].orthgene[Genotype::NVARS+2] - population[p].gene[Genotype::NVARS+2]) + 
			population[p].gene[Genotype::NVARS+1] * (temp1.gene[Genotype::NVARS+2] - temp2.gene[Genotype::NVARS+2]);
			if(MidPop[p].gene[Genotype::NVARS+2] > MidPop[p].upper[Genotype::NVARS+2])
				MidPop[p].gene[Genotype::NVARS+2] = MidPop[p].upper[Genotype::NVARS+2];
			else if(MidPop[p].gene[Genotype::NVARS+2] < MidPop[p].lower[Genotype::NVARS+2])
				MidPop[p].gene[Genotype::NVARS+2] = MidPop[p].lower[Genotype::NVARS+2];
			assert(MidPop[p].gene[Genotype::NVARS+2] >= MidPop[p].lower[Genotype::NVARS+2] &&   
				MidPop[p].gene[Genotype::NVARS+2] <= MidPop[p].upper[Genotype::NVARS+2]);
			population[p].useOrthCount = false;
		}

		if(population[p].unupdate >= MidPop[p].gene[Genotype::NVARS+2] && population[p].lBest != bestIdx)
		{
			population[p].useOrthCount = true;
			orthnumber++;
			population[p].unupdate = 0;
			Genotype temp[row + 2];
			for(int i = 1;i <= row;i++){
				assert(i >= 1 && i <= row);
				for(int j = 1;j <= Genotype::NVARS+3;j++){
					assert(j >= 1 && j <= Genotype::NVARS+3);
					if(oArray[i][j] == 1)
			...
			double tempfit[Genotype::NVARS+4][Genotype::LEVEL+1];
			Genotype ctemp;
			for(int i = 1;i <= Genotype::NVARS+3;i++){
				double min = 1;
				int index = -1;
		...
		else if(population[p].unupdate >= 5 && bestIdx == population[p].lBest){
			population[p].unupdate = 0;
			if(population[p].orthBestFit > bestA.fitness){
				memcpy(population[p].orthgene,bestA.gene,sizeof(bestA.gene));
				population[p].orthBestFit = bestA.fitness;
			}
		}
		MidPop[p].gene[Genotype::NVARS+1] = population[p].gene[Genotype::NVARS+1] + population[p].gene[Genotype::NVARS+1] * 
			(population[p].orthgene[Genotype::NVARS+1] - population[p].gene[Genotype::NVARS+1]) + 
		population[p].gene[Genotype::NVARS+1] * (temp1.gene[Genotype::NVARS+1] - temp2.gene[Genotype::NVARS+1]);
		if(MidPop[p].gene[Genotype::NVARS+1] < MidPop[p].lower[Genotype::NVARS+1]
				|| MidPop[p].gene[Genotype::NVARS+1] > MidPop[p].upper[Genotype::NVARS+1])
				MidPop[p].gene[Genotype::NVARS+1] = ranval(MidPop[p].lower[Genotype::NVARS+1],MidPop[p].upper[Genotype::NVARS+1]); 
		for(int j = 0;j <= Genotype::NVARS;j++){
			MidPop[p].gene[j] = population[p].gene[j] + MidPop[p].gene[Genotype::NVARS+1] * (population[p].orthgene[j] - population[p].gene[j]) + MidPop[p].gene[Genotype::NVARS+1] * (temp1.gene[j] - temp2.gene[j]);
...
int main(){
	...
	for(f = 1;f <= 13;f++){
		char filename[100];
		sprintf(filename,"/home/ryan/testdataNN/data_Scale0.5/MBC2013SACrO/outputc%d.txt",f);
		cout<<filename<<endl;
	...
	//				break;
			...
			output<<"updaterate "<<bestA.updateRate<<endl; 
			output<<"orthnumber "<<orthnumber<<endl;
		}
