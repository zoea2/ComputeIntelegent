...
void select(){
	for(int i = 1;i <= POPSIZE;i++){
		...
		/*	if((population[i].fitness != 0) && abs((MidPop[i].fitness - population[i].fitness) / population[i].fitness) * feNumber 
					> updateRate){
				updateRate = abs((MidPop[i].fitness - population[i].fitness) / population[i].fitness) * feNumber ;
...
int main(){
	...
	for(f = 1;f <= 13;f++){
		char filename[100];
		sprintf(filename,"/home/ryan/testdataNN/data_Scale%.1lf/MBC2013SACr/outputc%d.txt",Genotype::SCALE,f);
		cout<<filename<<endl;
