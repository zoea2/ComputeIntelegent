...
void cross(){
	for(int i = 1;i <= POPSIZE;i++){
		...
		for(int j = 0;j < Genotype::NVARS;j++){
			if(ranval(0,1) > Genotype::PXOVER && j != jrand){
...
int main(){
	...
	for(f = 1;f <= 13;f++){
		char filename[100];
		sprintf(filename,"/home/ryan/testdataNN/data_Scale%.1lf/MBC2013%.1lf/outputc%d.txt",Genotype::SCALE,Genotype::PXOVER,f);
		cout<<filename<<endl;
