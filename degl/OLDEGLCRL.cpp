...
void select(){
	for(int i = 1;i <= POPSIZE;i++){
		if(MidPop[i].fitness < population[i].fitness){
			...
			memcpy(population[i].gene,MidPop[i].gene,sizeof(MidPop[i].gene));
...
void cross(){
	for(int i = 1;i <= POPSIZE;i++){
		int jrand = (int)ranval(0,Genotype::NVARS);
		for(int j = 0;j < Genotype::NVARS;j++){
			if(ranval(0,1) > population[i].PXOVER && j != jrand){
...
int main(){
	...
	for(f = 1;f <= 13;f++){
		char filename[100];
		sprintf(filename,"/home/ryan/testdataNN/data_Scale%.1lf/MBC2013LinCr/outputc%d.txt",Genotype::SCALE,f);
		cout<<filename<<endl;
	...
	//				break;
				gen++;
				for(int i = 1;i <= POPSIZE;i++){
					population[i].genCrLin(feNumber,funcEvaluate);	
				}
			}
